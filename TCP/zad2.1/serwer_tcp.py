...
def server(port):
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            port = s.getsockname()[
                1
            ]  # w razie gdyby było 0 to port jest jakoś przydzielany, więc wypada wiedzieć na jakim porcie serwer nasłuchuje
            ...
            while True:
                ...
                with conn:
                    print(
                        "Connect from: {}:{}".format(addr[0], addr[1])
                    )  # w client_tcp.py można by bind-em przypisać mu sztucznie jakiś adres i port wtedy będzie widać różnicę, bo tak to będzie 127.0.0.1:losowy port
                    while True:
                        ...
                        try:
                            conn.sendall(
                                b"Data received!"
                            )  # wysyłamy odpowiedź, że otrzymaliśmy dane
